...
import { TicTacToeGame } from '@prisma/client';

@Injectable()
export class TicTacToeService {
    constructor(private prisma: PrismaService) { }

    async createGame(playerId: string): Promise<TicTacToeGame> {
        return this.prisma.ticTacToeGame.create({
            data: { playerId },
        });
    }

    async getGame(gameId: string): Promise<TicTacToeGame | null> {
        return this.prisma.ticTacToeGame.findUnique({ where: { id: gameId } });
    }

    async makeMove(gameId: string, playerMoveIndex: number): Promise<TicTacToeGame> {
        ...
        if (this.checkWin(boardArr, 'X')) {
            return this.prisma.ticTacToeGame.update({
                where: { id: gameId },
